...
#
#
  if (regexpr("PFAS", get_physchem_param("Chemical.Class",dtxsid=dtxsid,chemdata=chemdata)) != -1)
  {
    PFAS <- TRUE
  ...
  Params[["Qgfrc"]] <- (1-resorption.factor)*QGFRc/1000*60 #        L/h/kgBW  
  Params[["resorption.factor"]] <- resorption.factor 
...
# don't explicitly model blood from the gut:
  ...
  return(lapply(Params[model.list[["sumclearancespfas"]]$param.names],
                set_httk_precision))
}
