                        ...
                        chemdata = chem.physical_and_invitro.data)
{
  ...
  for (i in 1:num.chems)
  {
    ...
    if ((!is.null(this.chem.cas) & !any(chemdata$CAS==this.chem.cas)) & 
      (!is.null(this.chem.name) & !any(chemdata$Compound==this.chem.name)) &
      (!is.null(this.dtxsid) & !any(chemdata$DTXSID==this.dtxsid)))
    {
    ...
    if (!is.null(this.chem.name)){
      ...
      if (!any(names.index==name.key)) 
        stop ("Chemical name not found, use get_cheminfo(info=\"compound\") for \valid compound names.")
    ...
    if (!is.null(this.dtxsid)){
      ...
      if (!(this.dtxsid.key %in% tolower(chemdata$DTXSID))) 
        stop("this.dtxsid not found, use get_cheminfo(info=\"DTXISD\") for valid \this.dtxsids.\n")
  
      #Set the chemical name:
      found.chem.name <- as.character(na.omit(chemdata[tolower(chemdata[,"DTXSID"])==tolower(this.dtxsid.key),"Compound"]))
      found.chem.cas <- as.character(na.omit(chemdata[tolower(chemdata[,"DTXSID"])==tolower(this.dtxsid.key),"CAS"])) 
    }

    # Map "character(0)" back to NULL:
    if (identical(found.chem.name, character(0))) 
      found.chem.name <- NULL
    
    if (identical(found.dtxsid, character(0))) 
      found.dtxsid <- NULL
    
    if (identical(found.chem.cas, character(0))) 
      found.chem.cas <- NULL
  
    if (!is.null(found.chem.cas) & !is.null(found.chem.name) & !is.null(found.dtxsid)){
      
      if (!is.null(this.chem.cas)){
        if (!is.na(found.chem.cas)){
          if (this.chem.cas != found.chem.cas) 
            stop(paste("Both CAS",this.chem.cas,"and either name or this.dtxsid were provided as arguments, but found other CAS -- ",found.chem.cas))
        }
      } 
      else if (!is.null(this.dtxsid)){
        if (!is.na(found.dtxsid)){
          if (this.dtxsid != found.dtxsid) 
            stop(paste("Both this.dtxsid",this.dtxsid,"and either CAS or name were provided as arguments, but found other this.dtxsid -- ",found.dtxsid))
        }
      } 
      else if (!is.null(this.chem.name)){
        if (!is.na(found.chem.name)){
          if (tolower(this.chem.name) != tolower(found.chem.name)) 
            stop(paste("Both name",this.chem.name,"and either CAS or this.dtxsid were provided as arguments, but found other name -- ",found.chem.name))
        }
      }
    }
    
    if (is.null(this.chem.cas)) 
      this.chem.cas <- found.chem.cas
    
    if (is.null(this.chem.name)) 
      this.chem.name <- found.chem.name
    
    if (is.null(this.dtxsid))
      this.dtxsid <- found.dtxsid
  ...
  return(list(chem.cas=chem.cas.out,
              chem.name=chem.name.out,
              dtxsid=dtxsid.out)
 )
}
